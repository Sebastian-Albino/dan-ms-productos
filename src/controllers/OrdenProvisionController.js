...
async function listarOrdenesPorIdProveedor(req, res) {
  try {
    ...
    // console.log(idProv);
    const ordenes = await ordenProvisionService.listarOrdenesPorProveedor(
      idProv
    );
    return res.status(200).json(ordenes);
  } catch (error) {
...
async function listarOrdenesPorFecha(req, res) {
  try {
    ...
    const orden = await ordenProvisionService.listarOrdenesPorFecha(
      fechaDesde,
      fechaHasta
    );
    return res.status(200).json(orden);
...
async function modificarOrden(req, res) {
  try {
    const idOrden = req.params.id;
    const orden = req.body;
    const ordenResultante = await ordenProvisionService.modificarOrden(
      idOrden,
      orden
    );
...
async function modificarEstadoOrden(req, res) {
  try {
    ...
    const orden = await ordenProvisionService.modificarEstadoOrden(
      idOrden,
      fechaRecep
    );
...
async function actualizarStockProds(idOrden) {
  try {
    ...
    detalles.forEach(async (detalle) => {
      const prod = await productoService.modificarStockProducto(
        detalle.productoId,
        detalle.cantidad
      );
...
export default {
  ...
  listarOrdenesPorId,
  listarOrdenesPorIdProveedor,
  modificarOrden,
  modificarEstadoOrden,
  actualizarStockProds,
};
